...
#ifndef __OSM_REQUESTLAUNCHER_H__
#define __OSM_REQUESTLAUNCHER_H__
...
public:
    // For SLR7
    class Exception : public _NOSM::Exception
    {
    private:
        _NOSM::Exception e;

    public:
        Exception(const _NOSM::Exception& e)
        {
            this->e = e;
        }

        inline const _NOSM::Exception& GetInternalException() const
        {
            return this->e;
        }
    };
...
public:
    ...
    JsonArray getNearestRoadsForPath(VehiclePosVec poses); // throw Exception // SLR7
    JsonArray getWayNodes(double lat, double lon, double areaSize);
};

}

#endif // __OSM_REQUESTLAUNCHER_H__
