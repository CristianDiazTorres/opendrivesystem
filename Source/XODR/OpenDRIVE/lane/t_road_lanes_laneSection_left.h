...
#include "XODR/OpenDRIVE/lane/t_road_lanes_laneSection_left_lane.h"
...
#define _OD_MEMBERS(usingType, lineEnd) \
    ...
    _UNIVERSIAL_TYPE_SEQUENCE(usingType, 1, x, t_road_lanes_laneSection_left_lane, lane) lineEnd \
...
public:
    ...
    _UNIVERSIAL_TYPE_parent_isComp_opENE_RW_TEST_DEFINE(t_road_lanes_laneSection_left)
...
public:
    ...
    {
        qDebug() << "t_road_lanes_laneSection_left" << "          : updateGeoJSONData_Body";
        if (geojsonData.tag == "t_road_lanes_laneSection_left")
        {
    ...
    bool updateGeoJSONData_Children_Pre(_GeoJSONData& geojsonData, bool& inserted)
    {
        if (geojsonData.condPropsObj.contains("_t_road_lanes_laneSection_lcr_lane"))
        {
            ...
            if (condPropsObj.contains("id"))
            {
                ...
                return lane_sequence[lane_sequence.size() - id].updateGeoJSONData(geojsonData, inserted);
            }
        }

        double t = fabs(geojsonData.st.t);
        double offset = 0;
        for (t_road_lanes_laneSection_left_lane& lane : lane_sequence)
        {
        ...
        t_road_lanes_laneSection_left_lane lane;
    ...
    bool addRealLanes(int addedCount, const t_road_lanes_laneSection_lr_lane& referLane)
    {
        ...
        // We will add lanes left side (at begin)
        QVector<t_road_lanes_laneSection_left_lane>::iterator iter = lane_sequence.begin();

        // Skip first border lane
        if (!lane_sequence.isEmpty() && lane_sequence.first().type == e_laneType::BORDER)
            iter ++;
        ...
        // Add border lane left side (at begin)
        if (lane_sequence.first().type != e_laneType::BORDER)
        {
            t_road_lanes_laneSection_left_lane border;
    ...
    const t_road_lanes_laneSection_left_lane& getOutsideRealLane() const
    {
        for (int i = 0; i < lane_sequence.size(); i ++)
        {
            if (lane_sequence[i].type != e_laneType::BORDER)
    ...
    t_road_lanes_laneSection_left_lane& getOutsideRealLane()
    {
        for (int i = 0; i < lane_sequence.size(); i ++)
        {
            if (lane_sequence[i].type != e_laneType::BORDER)
    ...
    const t_road_lanes_laneSection_left_lane& getInsideRealLane() const
    {
    ...
    t_road_lanes_laneSection_left_lane& getInsideRealLane()
    {
        for (int i = lane_sequence.size() - 1; i >= 0; i --)
        {
            if (lane_sequence[i].type != e_laneType::BORDER)
    ...
    {
        // Remove right side border lane (at last)
        if (lane_sequence.last().type == e_laneType::BORDER)
        {
