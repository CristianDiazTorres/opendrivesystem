...
uint XmlStreamReaderPrivate::getChar_helper()
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < i; ++j) {
            if (attributes[j].name() == attribute.name()
                ...
                raiseWellFormedError(XmlStream::tr("Attribute '%1' redefined.").arg(attribute.qualifiedName().toString()));
        }
    }
...
/*!
    \fn XmlStreamAttributes::XmlStreamAttributes()

    A constructor for XmlStreamAttributes.
*/
...
/*! \fn XmlStreamStringRef::swap(XmlStreamStringRef &other)
    \since 5.6
